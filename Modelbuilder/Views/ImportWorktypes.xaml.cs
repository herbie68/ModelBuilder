...
/// <summary>
/// Interaction logic for ImportWorktypes.xaml
/// </summary>
public partial class ImportWorktypes : Page
{
    private HelperGeneral _helperGeneral;
    private HelperClass _helper;
    private ErrorClass _error;

    public ImportWorktypes()
    {
        InitializeComponent();
        InitializeHelper();
    }
        ...
        foreach (string line in lines)
        {
            if (ImportFailed == 0)
            {
                ...
                else
                {
                    ...
                    if (_helperGeneral.CheckForRecords(HelperGeneral.DbWorktypeTable, new string[1, 3]
                    {   { HelperGeneral.DbWorktypeTableFieldNameId, HelperGeneral.DbWorktypeTableFieldTypeId, lineField[Col[0]].ToString() }    }) != 0) { error = 2; ErrorCause = lineField[Col[0]].ToString(); }

                    // Parent Id has to exist
                    if (_helperGeneral.CheckForRecords(HelperGeneral.DbCurrencyTable, new string[1, 3]
                    {   { HelperGeneral.DbWorktypeTableFieldNameParentId, HelperGeneral.DbWorktypeTableFieldTypeParentId, lineField[Col[1]].ToString() }    }) == 33) { error = 32; ErrorCause = lineField[Col[1]].ToString(); }
                    #endregion

                    if (error == 0)
                    {
                        _helperGeneral.InsertInTable(HelperGeneral.DbWorktypeTable, new string[4, 3]
                        {
                            { HelperGeneral.DbWorktypeTableFieldNameId,    HelperGeneral.DbWorktypeTableFieldTypeId, lineField[Col[0]].ToString()},
                            { HelperGeneral.DbWorktypeTableFieldNameParentId,  HelperGeneral.DbWorktypeTableFieldTypeParentId, lineField[Col[1]].ToString()},
                            { HelperGeneral.DbWorktypeTableFieldNameName,    HelperGeneral.DbWorktypeTableFieldTypeName, lineField[Col[2]].ToString()},
                            { HelperGeneral.DbWorktypeTableFieldNameFullpath,    HelperGeneral.DbWorktypeTableFieldTypeFullpath, lineField[Col[3]].ToString()}
                        });
                    }
                    else
                    {
    ...
    private void PrepareEmptyCSV(object sender, RoutedEventArgs e)
    {
        ...
        _helper = new HelperClass();
        var FileName = _helper.GetFilePrefix() + Languages.Cultures.ExportWorktypes_FileName + ".csv";
        string[] Columns = _helper.GetCurrencyHeaders();
