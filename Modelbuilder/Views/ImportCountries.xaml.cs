...
/// <summary>
/// Interaction logic for ImportCountries.xaml
/// </summary>
public partial class ImportCountries : Page
{
    ...
    private void ClickedSelectFileButton(object sender, RoutedEventArgs e)
    {
        ...
        // 0 = Code, 1 = Name, 2 = CurrencySymbol, 3 = CurrencyId 
        int[] Col = { 0, 1, 2, 3};
        ...
        foreach (string line in lines)
        {
            if (ImportFailed == 0)
            {
                ...
                else
                {
                    ...
                    // Country Code must be non existing
                    if (_helperGeneral.CheckForRecords(HelperGeneral.DbCountryTable, new string[1, 3]
                    {   { HelperGeneral.DbCountryTableFieldNameCode, HelperGeneral.DbCountryTableFieldTypeCode, lineField[Col[0]].ToString() }    }) != 0) { error = 11; ErrorCause = lineField[Col[0]].ToString(); }

                    // Currency Id has to exist
                    if (_helperGeneral.CheckForRecords(HelperGeneral.DbCurrencyTable, new string[1, 3]
                    {   { HelperGeneral.DbCurrencyTableFieldNameId, HelperGeneral.DbCurrencyTableFieldTypeId, lineField[Col[3]].ToString() }    }) == 0) { error = 30; ErrorCause = lineField[Col[0]].ToString(); }
                    #endregion

                    if (error == 0)
                    {
                        _helperGeneral.InsertInTable(HelperGeneral.DbCountryTable, new string[4, 3]
                        {
                            { HelperGeneral.DbCountryTableFieldNameCode,    HelperGeneral.DbCountryTableFieldTypeCode, lineField[Col[0]].ToString()},
                            { HelperGeneral.DbCountryTableFieldNameName,  HelperGeneral.DbCountryTableFieldTypeName, lineField[Col[1]].ToString()},
                            { HelperGeneral.DbCountryTableFieldNameDefCurrencySymbol,    HelperGeneral.DbCountryTableFieldTypeDefCurrencySymbol, lineField[Col[2]].ToString()},
                            { HelperGeneral.DbCountryTableFieldNameDefCurrencyId,    HelperGeneral.DbCountryTableFieldTypeDefCurrencyId, lineField[Col[3]].ToString()}
                        });
                    }
                    else
                    {
    ...
    private void PrepareEmptyCSV(object sender, RoutedEventArgs e)
    {
        ...
        _helper = new HelperClass();
        var FileName = _helper.GetFilePrefix() + Languages.Cultures.ExportCountries_FileName + ".csv";
        string[] Columns = _helper.GetCurrencyHeaders();
