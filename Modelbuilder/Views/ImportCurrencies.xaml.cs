...
/// <summary>
/// Interaction logic for ImportCurrencies.xaml
/// </summary>
public partial class ImportCurrencies : Page
{
    ...
    private void ClickedSelectFileButton(object sender, RoutedEventArgs e)
    {
        ...
        // 0 = Code, 1 = Symbol, 2 = Name, 3 = ConversionRate 
        int[] Col = { 0, 1, 2, 3};
        ...
        foreach (string line in lines)
        {
            if (ImportFailed == 0)
            {
                ...
                else
                {
                    ...
                    // Currency Code must me non existing
                    if (_helperGeneral.CheckForRecords(HelperGeneral.DbCurrencyTable, new string[1, 3]
                    {   { HelperGeneral.DbCurrencyTableFieldNameCode, HelperGeneral.DbCurrencyTableFieldTypeCode, lineField[Col[0]].ToString() }    }) != 0) { error = 10; ErrorCause = lineField[Col[0]].ToString(); }
                    #endregion

                    if (error == 0)
                    {
                        _helperGeneral.InsertInTable(HelperGeneral.DbCurrencyTable, new string[4, 3]
                        {
                            { HelperGeneral.DbCurrencyTableFieldNameCode, HelperGeneral.DbCurrencyTableFieldTypeCode, lineField[Col[0]].ToString()},
                            { HelperGeneral.DbCurrencyTableFieldNameSymbol, HelperGeneral.DbCurrencyTableFieldTypeSymbol, lineField[Col[1]].ToString()},
                            { HelperGeneral.DbCurrencyTableFieldNameName, HelperGeneral.DbCurrencyTableFieldTypeName, lineField[Col[2]].ToString()},
                            { HelperGeneral.DbCurrencyTableFieldNameRate, HelperGeneral.DbCurrencyTableFieldTypeRate, lineField[Col[3]].ToString()}
                        });
                    }
                    else
                    {
    ...
    private void PrepareEmptyCSV(object sender, RoutedEventArgs e)
    {
        ...
        _helper = new HelperClass();
        var FileName = _helper.GetFilePrefix() + Languages.Cultures.ExportCurrencies_FileName + ".csv";
        string[] Columns = _helper.GetCurrencyHeaders();
